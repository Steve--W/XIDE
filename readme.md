...
Overview
--------
This Lazarus project provides a RAD IDE for Free Pascal and/or Python in the browser (Chrome) or desktop.
(Lazarus downloads:   https://wiki.lazarus.freepascal.org/fpcupdeluxe)
XIDE is developed and tested on Lazarus version 3.4, FPC version 3.2.2.
...
code is needed to provide extensive client side functionality and/or you need to share a code base between browser 
...
SVG components. The UI widget event handlers are then written in Pascal.  Python event handlers and code scripts can also be created.

Installation
------------

The package XComponents must first be installed into the Lazarus IDE. 
XComponents can be downloaded from: https://github.com/Steve--W/XComponents.  See separate readme notes for XComponents.
...
The compiler directive ‘-dChromium’ must be set in ‘Project Options’ to include these components (in both the XIDE project and the XComponents package).
...
If you want to write event handlers using Pascal, set the full path to your local Lazarus-installed pascal compiler, fpc.exe.  
Otherwise leave it blank, and see notes below under 'Python'.
...
Click the 'Edit Event Code' button (...) to pop up an editor, where a block of pascal or python code can be written.
Note there is no function header - this is handled automatically by the system builder.

See the 'Help' in the code editor, or main menu Help>Overview screen for notes on available functions within events code.

To test your code, use main menu 'Run Mode'.  For Pascal this will compile all your event code, and if there are
...
It also allows complete pascal code units or Python scripts to be added, to provide any common functions that may be required.

Python
------

To use Python for event handling code, the compiler option -dPython must be set.  Python event code is executed when the event is triggered.
Other Python scripts are executed on entry to run mode.
